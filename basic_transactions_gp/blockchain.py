...
def mine():
    ...
    if blockchain.valid_proof(last_block_string, submitted_proof):
        ...
        blockchain.add_transaction("0", values.get('id'), 1)
...
def new_transactions():
    ...
    next_block = blockchain.add_transaction(values['sender'], values['recipient'], values['amount'])

    response = {
        'message': f'Transaction added to block {next_block}'
    }
    return jsonify(response), 200
